...
_state: dict = {
    "status": "READY",
    # READY | DETECTING | COUNTDOWN | CAPTURING | CONVERTING | PLOTTING | DONE | ERROR
    "countdown": 0.0,
...
_settings: dict = {
    ...
    "pose_seconds": 5.0,
    # Plot dimensions (mm)  — default A4 portrait
...
def _camera_thread() -> None:
    ...
    while not _stop_camera.is_set():
        ...
        busy = cur_status in ("CAPTURING", "CONVERTING", "PLOTTING")
...
def _capture_worker(frame: np.ndarray, results, s: dict) -> None:
    ...
    try:
        ...
        # No margin — drawing starts exactly at (0, 0)
        if "margin_mm"     in _cfg_params: cfg_kwargs["margin_mm"]     = 0.0
        # Override internal max limits so user-specified sizes are never clamped
        if "max_width_mm"  in _cfg_params: cfg_kwargs["max_width_mm"]  = w_mm
        if "max_height_mm" in _cfg_params: cfg_kwargs["max_height_mm"] = h_mm
...
def _border_test(s: dict) -> None:
    ...
    w        = float(s.get("target_width_mm")  or 210)
    h        = float(s.get("target_height_mm") or s.get("target_width_mm") or 210)
...
_BOOL_KEYS  = {"mirror", "edge_only", "auto_plot"}
